...
(function () {
  ...
  function fade(_ref4) {
    var element = _ref4.element,
      ...
      visibleDisplayValue =
        _ref4$visibleDisplayV === void 0 ? 'block' : _ref4$visibleDisplayV;
    var domFaderId =
      element.dataset.domFaderId || (Date.now() * Math.random()).toFixed(0);
    ...
    var fadeDirection =
      direction ||
      (currentDisplay === 'none' ||
      element.classList.contains('DOM-fader-removed', 'DOM-fader-hidden')
        ? 'in'
        : 'out');
    ...
    if (
      fadeDirection === 'in' &&
      currentDisplay !== 'none' &&
      !element.classList.contains('DOM-fader-hidden')
    ) {
      return Promise.resolve(element);
    }
    ...
    return new Promise(function (resolve, reject) {
      setTimeout(
        function () {
          ...
          if (fadeDirection === 'in') {
            element.style.display = cachedStyle.display
              ? cachedStyle.display
              : visibleDisplayValue;
            element.style.opacity = cachedStyle.opacity
              ? cachedStyle.opacity
              : '1';
          }
        ...
        setTimeout(function () {
          if (fadeDirection === 'out') {
            element.classList.add(
              preventDisplayNone ? 'DOM-fader-hidden' : 'DOM-fader-removed'
            );
          }
          ...
          styleCache[domFaderId].display =
            currentDisplay !== 'none' ? currentDisplay : visibleDisplayValue;
          styleCache[domFaderId].opacity =
            currentOpacity === '0' ? '1' : currentOpacity;
          resolve(element);
        }, speed);
      });
    });
  ...
  function initPrintStyles() {
    ...
    function showContent() {
      ...
      hiddenElements.forEach(function (element) {
        element.classList.remove('DOM-fader-removed', 'DOM-fader-hidden');
        hiddenStatusCache[element.dataset.domFaderId] =
          element.dataset.classList.value;
      });
    }
    ...
    function hideContent() {
      hiddenElements.forEach(function (element) {
        element.classList.add(
          hiddenStatusCache[element.dataset.domFaderId].split(' ')
        );
      });
    }
